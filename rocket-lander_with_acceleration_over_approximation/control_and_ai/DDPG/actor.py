...
class Actor():
        ...
        if explore:
            for action in actions:
                for i in range(len(action)):
                    ...
                    if i == 0:
                        action[i] = max(min(action[i], 1.0), 0.0)
                    else:
    ...
    #         fc2 = batch_layer(fc1, 400, 400, phase, name+'Layer-1', nonlinearity=tf.nn.relu)
    ...
    #     with tf.variable_scope(name + '_fc_2'):
    #         fc2 = layer(fc1, 300, activation=tf.nn.relu)
    #     with tf.variable_scope(name + '_fc_3'):
    ...
    def create_nn(self, state, name='actor'):
        with tf.variable_scope(name + '_fc_1'):
            fc1 = layer(state, 20, activation=tf.nn.relu)
        with tf.variable_scope(name + '_fc_2'):
            fc2 = layer(fc1, 20, activation=tf.nn.relu)
        with tf.variable_scope(name + '_fc_3'):
            fc3 = layer(fc2, 20, activation=tf.nn.relu)
        with tf.variable_scope(name + '_fc_4'):
            fc4 = layer(fc3, 20, activation=tf.nn.relu)
        with tf.variable_scope(name + '_fc_5'):
            fc5 = layer(fc4, 20, activation=tf.nn.relu)
        with tf.variable_scope(name + '_fc_7'):
            fc6 = layer(fc5, self.action_space_size, is_output=True)

        return tf.tanh(fc6) * self.action_space_bounds + np.array([0.5, 0.0, 0.0])
